...
class ManagerAgent(BaseAgent):
    ...
    def __action_parser__(self, raw_action: str) -> AgentAct:
        ...
        agent_act = None  # Initialize agent_act
        ...
        # if action_name is action and we haven't created agent_act yet
        ...
        # If we still haven't created agent_act, create a default Think action
        if agent_act is None:
            agent_act = AgentAct(name="Think", params={"response": "I need to think about what to do next."})
    ...
    def forward(self, task: TaskPackage, agent_act: AgentAct) -> str:
        ...
        for agent in self.team:
            if self.agent_match(agent_act.name, agent):
                ...
                # If this is a case presentation, return it directly
                if isinstance(observation, dict) and "case_presentation" in observation:
                    return observation
